# 🌡️ OTGW Logger & Monitor (Ampera/Laxilef Edition)

Умный мониторинг для котлов **Baxi Ampera Pro** (и аналогов) через шлюз **OpenTherm Gateway** с прошивкой **Laxilef**.

Скрипт решает главную проблему этой связки: он объединяет данные из разных источников (стандартный OpenTherm, текстовые логи датчиков Laxilef, MQTT-контекст) в единый поток данных для Home Assistant и Telegram.
